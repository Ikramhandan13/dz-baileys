            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    try {
                        ...
                        // Auto-Repair: if the session is corrupted (Bad MAC or session error), delete the session to trigger renegotiation
                        const errMessage = err?.message || err?.toString() || '';
                        if (errMessage.includes('Bad MAC') || isSessionRecordError(err)) {
                            logger.warn({ jid: decryptionJid }, 'Auto-repair: clearing corrupted session to trigger renegotiation');
                            try {
                                await repository.deleteSession([decryptionJid]);
                            } catch (e) {
                                logger.error({ err: e, jid: decryptionJid }, 'Auto-repair: failed to clear corrupted session');
                            }
                        }
                    }
                }
