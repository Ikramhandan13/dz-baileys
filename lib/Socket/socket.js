...
export const makeSocket = (config) => {
    ...
    return {
        ...
        lidToJid: async (lid) => {
            ...
            try {
                const pn = await signalRepository.lidMapping.getPNForLID(lid);
                return pn || lid;
            } catch (err) {
                logger.error({ err, lid }, 'failed to resolve lid to jid');
                return lid;
            }
        },
        lidToJidSync: (lid) => {
            ...
            try {
                ...
                const cached = signalRepository.lidMapping.mappingCache.get(`lid:${lidUser}`);
                if (cached && typeof cached === 'string') {
                    const decoded = jidDecode(lid);
                    const lidDevice = decoded?.device !== undefined ? decoded.device : 0;
                    return `${cached}:${lidDevice}@s.whatsapp.net`;
                }
                return lid;
            } catch (err) {
                return lid;
